...
from tokenizer.tokenizer_image.vq_loss_second_stage import VQ_models
...
def main(args):
    ...
    vq_model.quantize.set_train_stage("second")
    del checkpoint
    ...
    all_save_samples = [None for _ in range(world_size)]
    dist.all_gather_object(all_save_samples, save_samples)
    dist.barrier()
    if rank == 0:
        merged_samples = []
        for samples in all_save_samples:
            merged_samples.extend(samples)
        merged_samples = np.concatenate(merged_samples, axis=0)[:num_fid_samples]
        npz_path = f"{sample_folder_dir}.npz"
        np.savez(npz_path, arr_0=merged_samples)
