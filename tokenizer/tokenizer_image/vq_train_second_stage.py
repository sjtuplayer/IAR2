...
from tokenizer.tokenizer_image.vq_loss_second_stage import VQLoss
...
def main(args):
    ...
    embedding_params = [vq_model.quantize.embedding.weight]
    embedding_second_params = [vq_model.quantize.embedding_second]
    other_params = [p for n, p in vq_model.named_parameters()
                    if 'quantize.embedding.weight' not in n
                    and 'quantize.embedding_second' not in n]
    optimizer_embedding = torch.optim.Adam(embedding_params, lr=args.lr, betas=(args.beta1, args.beta2))
    optimizer_embedding_second = torch.optim.Adam(embedding_second_params, lr=args.lr, betas=(args.beta1, args.beta2))
    optimizer_main = torch.optim.Adam(other_params, lr=args.lr, betas=(args.beta1, args.beta2))
    ...
    if args.vq_ckpt:
        ...
        pretrained_dict = {k: v for k, v in pretrained_dict.items() if k in model_dict and "embedding_second" not in k}
        model_dict.update(pretrained_dict)
        vq_model.load_state_dict(model_dict)
    ...
    for epoch in range(start_epoch, args.epochs):
        ...
        for x, y in loader:
            stage_name, stage_total = stage_cycle[stage_pointer]
            if stage_iter_count == 0:
                vq_model.module.quantize.set_train_stage(stage_name)
            stage_iter_count += 1
            if stage_iter_count == stage_total:
                stage_pointer = (stage_pointer + 1) % len(stage_cycle)
                stage_iter_count = 0
            ...
            if stage_name == "first":
                ...
                with torch.cuda.amp.autocast(dtype=ptdtype):
                    ...
                    loss_gen1 = vq_loss(codebook_loss, imgs, recons_imgs, optimizer_idx=0, global_step=train_steps + 1,
                                        last_layer=vq_model.module.decoder.last_layer,
                                        logger=logger, log_every=args.log_every, stage="first")
                    vq_model.module.quantize.set_train_stage("second")
                scaler.scale(loss_gen1).backward()
                if args.max_grad_norm != 0.0:
                    scaler.unscale_(optimizer_embedding)
                    torch.nn.utils.clip_grad_norm_(embedding_params, args.max_grad_norm)

            optimizer_embedding_second.zero_grad()
            optimizer_main.zero_grad()
            with torch.cuda.amp.autocast(dtype=ptdtype):
                recons_imgs, codebook_loss = vq_model(imgs)
                loss_gen2 = vq_loss(codebook_loss, imgs, recons_imgs, optimizer_idx=0, global_step=train_steps + 1,
                                   last_layer=vq_model.module.decoder.last_layer,
                                   logger=logger, log_every=args.log_every, stage="second")
            scaler.scale(loss_gen2).backward()

            if args.max_grad_norm != 0.0:
                scaler.unscale_(optimizer_embedding_second)
                torch.nn.utils.clip_grad_norm_(embedding_second_params, args.max_grad_norm)
                scaler.unscale_(optimizer_main)
                torch.nn.utils.clip_grad_norm_(other_params, args.max_grad_norm)

            if stage_name == "first":
                scaler.step(optimizer_embedding)
            scaler.step(optimizer_embedding_second)
            ...
            # # Log loss values:
            running_loss += loss_gen2.item() + loss_disc.item()
            ...
            if train_steps % args.ckpt_every == 0 and train_steps > 0:
                if rank == 0:
                    ...
                    checkpoint = {
                        ...
                        "optimizer_embedding": optimizer_embedding.state_dict(),
                        "optimizer_embedding_second": optimizer_embedding_second.state_dict(),
...
if __name__ == "__main__":
    ...
    parser.add_argument("--results-dir", type=str, default="results_tokenizer_image_2st_stage_16dim_wol2")
