...
def main(args):
    ...
    for x, y in loader:
        ...
        codes1 = indices[0].reshape(x.shape[0], num_aug, -1)
        codes2 = indices[1].reshape(x.shape[0], num_aug, -1)

        x1 = codes1.detach().cpu().numpy()    # (1, num_aug, args.image_size//16 * args.image_size//16)
        x2 = codes2.detach().cpu().numpy()    # (1, num_aug, args.image_size//16 * args.image_size//16)
        train_steps = rank + total
        np.save(f'{args.code_path}/{args.dataset}{args.image_size}_codes/{train_steps}.npy', [x1,x2])
