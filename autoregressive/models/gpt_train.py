...
class Transformer(nn.Module):
    def __init__(self, config: ModelArgs):
        ...
        self.sub_vocab_size = config.sub_vocab_size
        ...
        self.mlp1 = nn.Sequential(
            nn.Linear(config.dim, config.dim//4),  # 降维
            nn.GELU(),
            nn.Linear(config.dim//4, config.dim//12, bias=False)  # 最终输出 c 维
        )
        self.mlp2 = nn.Sequential(
            nn.Linear(config.dim//12 * 12, config.dim*2),
            nn.GELU(),
            nn.Linear(config.dim*2, config.dim, bias=False)  # 最终输出 c 维
        )
